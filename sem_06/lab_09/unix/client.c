...
int main(int argc, char *argv[]) 
{
    if (argc < 3)
    {
        perror("There are not 2 extra params (sock_name and message)");
        return 1;
    }
    ...
    {
        perror("Failed to call sendto");
        return 1;
    }

    printf("Client sent: %s\n", buffer);
    bytes_recv = recvfrom(socket_fd, buffer, sizeof(buffer), 0, (struct sockaddr*) &server_address, &address_length);

    if (bytes_recv < 0 ) 
    {
        perror("Failed to call recvfrom()");
